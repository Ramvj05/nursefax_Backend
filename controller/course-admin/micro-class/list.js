...
router.post("/list", authorizer, async function (req, res) {
	...
	try {
		if (user.roles.includes("LIST_MICROCLASS") || user.roles.includes("ADMIN")) {
			...
			let microclass;
			let totalElements = await MicroClassModel.find(query).count();
			microclass = await pagination(MicroClassModel.find(query), page, pageSize);

			console.log(microclass);
			if (microclass.length > 0) {
				res
					.header({
