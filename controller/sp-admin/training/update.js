...
router.put("/update/:id", authorizer, async function (req, res) {
  ...
  try {
    ...
    if (user.roles.includes("ADMIN")) {
      const payload = new TrainingClass(req.body).getModel();

      console.log(payload);

      const updatedtest = await TrainingModel.findOneAndUpdate(
        {
          deleted: false,
          _id: id,
        },
      ...
      res
        ...
        .send({
          data: null,
          message: "You do not have access to modify training",
          statsCode: 401,
          error: {
