...
router.post("/list", authorizer, async function (req, res) {
  ...
  try {
    if (
      ...
      user.roles.includes("LIST_TRAINING") ||
      ...
      let totalElements = await TrainingModel.find(query).count();

      test = await await pagination(TrainingModel.find(query), page, pageSize);
      ...
      res
        ...
        .send({
          data: null,
          message: "You do not have access to get training",
          statsCode: 401,
          error: {
