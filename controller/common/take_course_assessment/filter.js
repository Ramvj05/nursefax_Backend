...
const AssessmentModel = require("../../../model/questionpool.model");
...
router.post("/filter", authorizer, async function (req, res) {
  ...
  try {
    ...
    if (
      ...
      let allExam = [];
      let exam;
      let totalElements = await TakeAssessmentModel.find(query).count();
      exam = await pagination(TakeAssessmentModel.find(query), page, pageSize);
